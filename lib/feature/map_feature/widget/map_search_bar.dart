          ...
          child: Material(
            ...
            child: Column(
              ...
              children: List.generate(
                mapBloc.state.resources.length,
                (index) => ListTile(
                  leading: const Icon(Icons.place),
                  title: Text(
                    '${mapBloc.state.resources[index].address!.formattedAddress}',
                    style: const TextStyle(
                        fontSize: 16, fontWeight: FontWeight.bold),
                  ),
                  subtitle: Text(
                    '${mapBloc.state.resources[index].address!.locality} ${mapBloc.state.resources[index].address!.adminDistrict} ${mapBloc.state.resources[index].address!.countryRegion}',
                    style: const TextStyle(fontSize: 10),
                  ),
                  onTap: () {
                    context.read<MapBloc>().add(PlaceSelectedViaSearchEvent(
                        mapBloc.state.resources[index]));
                    controller.close();
                  },
                ),
              ),
